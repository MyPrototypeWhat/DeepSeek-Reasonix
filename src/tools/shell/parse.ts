...
/** Issue #2105 — the no-shell executor can't expand env vars / command substitution / globs, so a
 *  command like `ls $DIR/*.ts` reaches the process verbatim and "silently produces wrong results".
 *  Flag the constructs a real shell WOULD expand (quote-aware; `~` excepted — see `expandTilde`). */
export function detectUnsupportedExpansion(
  cmd: string,
): { kind: "env" | "cmdsub" | "glob"; sample: string } | null {
  let quote: '"' | "'" | null = null;
  for (let i = 0; i < cmd.length; i++) {
    const ch = cmd[i]!;
    if (quote === "'") {
